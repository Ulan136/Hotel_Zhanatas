...
/* Категория проживания — как пишут в документах вахтовых гостиниц:
   ИТР (инженерно-технические работники) и вахтовый персонал. */
export const STAY_TYPES = ['ИТР', 'Вахтовый'];
export function stayTypeLabel(v) {
  const t = String(v || '').trim();
  return STAY_TYPES.includes(t) ? t : '';
}
/* Коротко — для плитки комнаты, где помещается три-пять букв.
   Возвращаем и метку, и ключ: по ключу подбирается цвет. */
export function stayTypeShort(v) {
  const t = stayTypeLabel(v);
  if (t === 'ИТР') return { key: 'itr', text: 'ИТР' };
  if (t === 'Вахтовый') return { key: 'vah', text: 'Вахта' };
  return { key: 'non', text: '—' };
}
...
/* Должности не придумываем заранее — берём те, что уже стоят в анкетах.
   Список пополняется сам: кто-то вписал новую через «Другое» — в следующий
   раз она уже будет в выпадающем списке. Повторы с разным регистром
   склеиваем, чтобы не было «Инженер» и «инженер» одновременно. */
export function positionsOf(guests) {
  const seen = new Map();
  for (const g of guests || []) {
    const p = String(g?.position || '').trim();
    if (!p) continue;
    const key = p.toLowerCase();
    if (!seen.has(key)) seen.set(key, p);
  }
  return [...seen.values()].sort((a, b) => a.localeCompare(b, 'ru')).concat('Другое');
}
